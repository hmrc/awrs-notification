...
trait ErrorConfig {
  ...
  lazy val invalidUnknown = configuration.getString("unknown.invalid").fold("")(x => x)
  lazy val invalidWErrorBuilder = configuration.getString("error_builder.invalid").fold("")(x => x)
  ...
  lazy val getError = (key: String) => configuration.getString(key).fold("")(x => x)
}
