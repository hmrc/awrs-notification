...
class NotificationCacheServiceTest extends BaseSpec with MockitoSugar with GuiceOneAppPerSuite {
  ...
  "NotificationCacheService" should {
    "return StatusNotification object when the notification is found in mongo" in {
      ...
      when(mockNotificationRepository.findByRegistrationNumber(any())).thenReturn(Future.successful(notification))
      val result = Await.result(notificationCacheService.findNotification("XXAW00000123488"), 2.second)
    ...
    "return None when the notification is found in mongo but not the correct contact type" in {
      val notification = Some(StatusNotification(
        Some("XXAW00000123488"), Some("123456789333"),
        Some(ContactTypes.OTHR), Some("04"), Some("2017-04-01T0013:07:11"))
      )
      when(mockNotificationRepository.findByRegistrationNumber(any())).thenReturn(Future.successful(notification))
      val result = Await.result(notificationCacheService.findNotification("XXAW00000123488"), 2.second)
    ...
    "return false when the notification is not stored in mongo because it is not the correct contact type (CONA)" in {
      ...
      val result = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
      result shouldBe false
    }

    "return false when the notification is not stored in mongo because it is not the correct contact type (OTHR)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.OTHR),
    ...
    "return false when the notification is not stored in mongo because it is not the correct contact type (NMRJ)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.NMRJ),
        contact_number = Some("123456789012"), variation = false
      )
      val result = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
      result shouldBe false
    ...
    "return true when the notification is stored in mongo (NMRJ)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.REJR),
        contact_number = Some("123456789012"), variation = false
      )
      when(mockNotificationRepository.insertStatusNotification(any())).thenReturn(Future.successful(true))
      val result: Boolean = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
      result shouldBe true
    }

    "return true when the notification is stored in mongo (REVR)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.REVR),
        contact_number = Some("123456789012"), variation = false
      )
      when(mockNotificationRepository.insertStatusNotification(any())).thenReturn(Future.successful(true))
      val result = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
    ...
    "return true when the notification is stored in mongo (MTRJ)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.MTRJ),
        contact_number = Some("123456789012"), variation = false
      )
      when(mockNotificationRepository.insertStatusNotification(any())).thenReturn(Future.successful(true))
      val result = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
    ...
    "return true when the notification is stored in mongo (MTRV)" in {
      val push = PushNotificationRequest(
        name = "name", email = "exampe@example.com", status = Some("04"), contact_type = Some(ContactTypes.MTRV),
        contact_number = Some("123456789012"), variation = false
      )
      when(mockNotificationRepository.insertStatusNotification(any())).thenReturn(Future.successful(true))
      val result = Await.result(notificationCacheService.storeNotification(push, "XXAW00000123488"), 2.second)
    ...
    "return true when the notification is deleted from mongo" in {
      ...
      when(mockNotificationRepository.deleteStatusNotification(any())).thenReturn(Future.successful(writeResult))
    ...
    "return the viewed status found in mongo when it exists" in {
      ...
      when(mockNotificationViewedRepository.findViewedStatusByRegistrationNumber(any())).thenReturn(Future.successful(viewedStatus))
      val result = Await.result(notificationCacheService.findNotificationViewedStatus("XXAW00000123488"), 2.second)
      result shouldBe viewedStatus
    }

    "return None when the viewed status is not found in mongo" in {
      when(mockNotificationViewedRepository.findViewedStatusByRegistrationNumber(any())).thenReturn(Future.successful(None))
      val result = Await.result(notificationCacheService.findNotificationViewedStatus("XXAW00000123488"), 2.second)
      result.isDefined shouldBe false
    }
    ...
    "return true when the viewed status is stored in mongo" in {
      ...
      val result = Await.result(notificationCacheService.storeNotificationViewedStatus(viewedStatus = false,
        "XXAW00000123488"), 2.second)
      result shouldBe true
    }

    "return true when the viewed status is updated in mongo" in {
      val writeResult = mock[UpdateResult]
      when(mockNotificationViewedRepository.markAsViewed(any())).thenReturn(Future.successful(writeResult))
      when(writeResult.wasAcknowledged()).thenReturn(true)
